...
class SliddeShow extends StatefulWidget {
  const SliddeShow({Key? key}) : super(key: key);

  @override
  State<SliddeShow> createState() => _SliddeShowState();
}

class _SliddeShowState extends State<SliddeShow> {
  @override
      ...
      body: LayoutBuilder(
        builder: (context,constrains){
          return  StreamBuilder<String>(
              stream: slideImage(MediaQuery.of(context).size.width.round(),
                  constrains.biggest.height.round()),
  ...
  Stream<String> slideImage(int width, int height) async* {
    log("width :: $width ::: height $height");
    yield "https://picsum.photos/$width/${(height).round()}";
    for (int i = 0; i < 10; i++) {
      await Future.delayed(const Duration(seconds: 10));
      yield "https://picsum.photos/${(width).round()}/${(height).round()}";
    }
  }
}
